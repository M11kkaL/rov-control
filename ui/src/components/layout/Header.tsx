...
export function Header({ mode, connected, ping, time, recording }: HeaderProps) {
  return (
...
function RecIndicator({ active }: { active: boolean }) {
  return (
    <div className="flex items-center gap-1.5">
      <span
        className={`h-2.5 w-2.5 rounded-full ${active ? 'animate-pulse bg-danger shadow-[0_0_8px_#ef4444]' : 'bg-white/20'}`}
      />
      <span className={`text-[10px] font-bold uppercase tracking-widest ${active ? 'text-danger' : 'text-white/30'}`}>
        Rec
      </span>
    </div>
  )
